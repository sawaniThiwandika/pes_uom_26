...
interface MemberCardProps {
  name: string; role: string; image: string; linkedin: string; email: string;
}
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0" style={{
        background: "linear-gradient(to top, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.22) 42%, transparent 70%)"
      }} />
      ...
      <div className="absolute top-2.5 left-2.5 sm:top-3 sm:left-3">
        <span className="text-[7px] sm:text-[8px] font-bold tracking-[0.16em] uppercase px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full border"
          style={{ background: "rgba(0,0,0,0.55)", backdropFilter: "blur(8px)",
            borderColor: "rgba(52,211,153,0.35)", color: "#6ee7b7" }}>
          {role}
        </span>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-2.5 sm:p-3.5 z-20">
        <h3 className="text-[11px] sm:text-[13px] font-semibold text-white leading-snug mb-2"
