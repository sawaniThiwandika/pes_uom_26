...
import {
  FaBolt, FaBars, FaTimes, FaHome, FaInfoCircle,
    ...
    <>
      ...
      <nav className={`nav-root ${hideBar ? "nav-hidden" : ""}`}>
        <div className={`nav-pill ${scrolled ? "pill-scrolled" : "pill-top"}`}>
          ...
          <button className="nav-hamburger" onClick={openMenu} aria-label="Open menu" aria-expanded={menuOpen}>
            <FaBars />
          </button>
        </div>
      </nav>
      ...
      >
        ...
        <div className="nav-panel-links">
          {NAV_LINKS.map(({ to, label, Icon }) => (
            <NavLink key={to} to={to} end={to === "/"}
              className={({ isActive }) => `nav-panel-link ${isActive ? "panel-active" : ""}`}
              onClick={closeMenu}>
              <div className="nav-panel-link-icon"><Icon /></div>
              {label}
            </NavLink>
          ))}
        </div>
        ...
        <div className="nav-panel-footer">
          <p style={{ fontSize: "10px", color: "rgba(255,255,255,0.2)", lineHeight: 1.6 }}>
            © {new Date().getFullYear()} IEEE PES SBC<br />University of Moratuwa
          </p>
        </div>
      </div>
    </>
